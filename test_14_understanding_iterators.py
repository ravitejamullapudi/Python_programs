...
notes = '''
Iterators are objects that represent a stream of data. next() method returns
the next available element. StopIteration is raised when elements are finished.

Python builtins like sequences, sets and dicts are iterable (ie) you can call iter(obj) on them
...
def test_find_builtins_that_work_on_iterables():
    funcs = ['all','any','filter','frozenset','list','map','max','min','next','reduce','set','sorted','sum','tuple','zip']
    assert 15 == len(funcs)


three_things_i_learnt = """
- didnt understand clearly
- all I know is datatypes which can support iterator can be used in loops kind of stuff
-
"""
